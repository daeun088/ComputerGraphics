...
void drawFootWithToes(float xOffset)
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        if (i == 1)
            glScalef(1.1f, 1.0f, 1.4f); // 가운데: 더 넓고 길게
        else
            glScalef(1.0f, 1.0f, 1.2f); // 양 옆: 기본
...
void display()
{
    ...
    // 양쪽 다리 그리기
    glPushMatrix();
    glTranslatef(-0.3f, -1.85f, 0.1f); // 왼쪽 다리 0.1f 위로
    glRotatef(180, 0, 1, 0);
    ...
    glPushMatrix();
    glTranslatef(0.3f, -1.85f, 0.1f); // 오른쪽 다리 0.1f 위로
    glRotatef(180, 0, 1, 0);
    glScalef(0.55f, 0.55f, 0.55f);
    drawFootWithToes(0.0f);
    glPopMatrix();
